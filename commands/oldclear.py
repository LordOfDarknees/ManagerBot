...
class oldclear(Cog):
    ...
    @slash_command(description="oldclears MSGs")
    async def oldclear(self, interaction: ApplicationCommandInteraction, number:int):
        if not await Owner().isOwner(self.bot, interaction.user.id):
            ...
            return
        wip_embed = Embed(title="oldclear Command", description="Im working on it!\nMay take a while!", color=Color.orange())
        ...
        for x in mgs:
            await x.delete()
        done_embed = Embed(title="oldclear Command", description="Everything is done now!", color=Color.green())
        await interaction.edit_original_response(embed=done_embed)
        


def setup(bot: Bot) -> None:
    bot.add_cog(oldclear(bot))